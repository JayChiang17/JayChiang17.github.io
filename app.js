...
const US_MIDDLE_INITIALS='ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('');
const NPB_NAME_SUFFIX=['也','人','介','郎','斗','真','生','希','悟','司','平','哉'];
function randomNpcName(org,used){
  used=used||new Set();let name='';
  for(let i=0;i<800;i++){
    if(org==='NPB')name=pick(NPB_SURNAMES)+pick(NPB_GIVEN)+(i>80?pick(NPB_NAME_SUFFIX):'');
    else if(org==='MiLB')name=pick(US_FIRST)+(i>50?' '+pick(US_MIDDLE_INITIALS)+'.':'')+' '+pick(US_SURNAMES);
    else name=pick(PROSPECT_SURNAMES)+pick(PROSPECT_GIVEN_A)+(chance(82)?pick(PROSPECT_GIVEN_B):'');
    if(!isPlayerName(name)&&!used.has(name)){used.add(name);return name;}
  }
  if(org==='MiLB')for(const first of US_FIRST)for(const initial of US_MIDDLE_INITIALS)for(const last of US_SURNAMES){name=`${first} ${initial}. ${last}`;if(!isPlayerName(name)&&!used.has(name)){used.add(name);return name;}}
  if(org==='NPB')for(const surname of NPB_SURNAMES)for(const given of NPB_GIVEN)for(const suffix of NPB_NAME_SUFFIX){name=surname+given+suffix;if(!isPlayerName(name)&&!used.has(name)){used.add(name);return name;}}
  do{name=pick(PROSPECT_SURNAMES)+pick(PROSPECT_GIVEN_A)+pick(PROSPECT_GIVEN_B);}while(isPlayerName(name)||used.has(name));used.add(name);return name;
}
function repairNpcSerialName(name){
  const text=String(name||''),m=text.match(/^(.*)\s(\d{2})$/);if(!m)return text;const base=m[1],code=Number(m[2])||0,parts=base.split(/\s+/);
  if(parts.length>=2)return `${parts[0]} ${US_MIDDLE_INITIALS[code%US_MIDDLE_INITIALS.length]}. ${parts.slice(1).join(' ')}`;
  return base+NPB_NAME_SUFFIX[code%NPB_NAME_SUFFIX.length];
}
function repairNpcNamesInState(state){
  if(!state)return;const fix=o=>{if(o&&typeof o.name==='string')o.name=repairNpcSerialName(o.name);};
  const world=state.leagueWorld||{};Object.values(world.rosters||{}).flat().forEach(fix);(world.history||[]).forEach(row=>{if(row&&row.name)row.name=repairNpcSerialName(row.name);if(row&&Array.isArray(row.players))row.players=row.players.map(repairNpcSerialName);});
  if(state.careerCast){fix(state.careerCast.rival);fix(state.careerCast.teammate);}fix(state.teammate);if(state.npcSeasonContext){fix(state.npcSeasonContext.rival);(state.npcSeasonContext.teammates||[]).forEach(fix);}world.version=4;
}
...
function npcWorld(){const world=S.leagueWorld||(S.leagueWorld={version:4,rosters:{},lastYear:0,history:[]});world.rosters=world.rosters||{};world.history=world.history||[];if((world.version||1)<4)repairNpcNamesInState(S);world.version=4;return world;}
...
function normalizeSavePackage(raw){
  ...
  if((state.leagueWorld.version||1)<3){state.leagueWorld.rosters={};state.leagueWorld.history=[];state.leagueWorld.lastYear=0;}repairNpcNamesInState(state);
...
const TEAM_BROADCAST_CODE={
  '巴爾的摩金鶯':'BAL','波士頓紅襪':'BOS','紐約洋基':'NYY','坦帕灣光芒':'TB','多倫多藍鳥':'TOR',
  '芝加哥白襪':'CWS','克里夫蘭守護者':'CLE','底特律老虎':'DET','堪薩斯市皇家':'KC','明尼蘇達雙城':'MIN',
  '運動家':'ATH','休士頓太空人':'HOU','洛杉磯天使':'LAA','西雅圖水手':'SEA','德州遊騎兵':'TEX',
  '亞特蘭大勇士':'ATL','邁阿密馬林魚':'MIA','紐約大都會':'NYM','費城費城人':'PHI','華盛頓國民':'WSH',
  '芝加哥小熊':'CHC','辛辛那提紅人':'CIN','密爾瓦基釀酒人':'MIL','匹茲堡海盜':'PIT','聖路易紅雀':'STL',
  '亞利桑那響尾蛇':'AZ','科羅拉多洛磯':'COL','洛杉磯道奇':'LAD','聖地牙哥教士':'SD','舊金山巨人':'SF',
  '東京大人':'YG','阪神猛虎':'HT','橫濱海星':'YB','廣島紅鯉':'HC','神宮飛燕':'YS','名古屋神龍':'CD',
  '福岡猛禽':'SH','北海道培根':'F','千葉海潮':'M','仙台金翼':'E','大阪蠻牛':'B','埼玉雄獅':'L',
  '台中猛瑪':'TCM','府城雄獅':'TNL','桃園金剛':'TYG','新北騎士':'NTK','台北恐龍':'TPD','高雄神鵰':'KAE'};
function teamBroadcastCode(name){return TEAM_BROADCAST_CODE[name]||teamMonogram(name);}
...
function careerTeamSummaryItems(){
  const map=new Map();(S.log||[]).filter(x=>x&&x.tm).forEach(row=>{const team=String(row.tm),item=map.get(team)||{team,first:Number(row.y)||S.year,last:Number(row.y)||S.year,years:new Set(),games:0};item.first=Math.min(item.first,Number(row.y)||S.year);item.last=Math.max(item.last,Number(row.y)||S.year);item.years.add(Number(row.y)||S.year);item.games+=(row.st&&Number(row.st.G))||0;map.set(team,item);});
  const current=S.stage==='PRO'?(S.orgTeam||'自由球員'):(S.team||stageLabel());if(current&&current!=='自由球員'&&!map.has(current))map.set(current,{team:current,first:S.year,last:S.year,years:new Set([S.year]),games:0,current:true});
  return [...map.values()].map(x=>({...x,seasons:x.years.size,range:x.first===x.last?String(x.first):`${x.first}–${x.last}`,current:x.team===current})).sort((a,b)=>a.first-b.first);
}
function careerHonorSummaryItems(){
...
function careerLegacyHTML(){
  const teams=careerTeamSummaryItems(),honors=careerHonorSummaryItems(),total=(S.honors||[]).length;
  const teamHTML=teams.map(x=>`<article class="summary-team-card${x.current?' current':''}" style="--team:${TEAM_COLOR[x.team]||'#648474'}"><em>${escapeHTML(teamBroadcastCode(x.team))}</em><div><small>${escapeHTML(x.range)}${x.current?'｜目前':''}</small><b>${escapeHTML(x.team)}</b><span>${x.seasons} 季${x.games?`・${x.games.toLocaleString()} 場`:''}</span></div></article>`).join('');
  const honorHTML=honors.map(x=>`<article class="summary-honor-row"><time>${x.first===x.last?x.first:`${x.first}–${x.last}`}</time><b>${escapeHTML(x.title)}</b>${x.count>1?`<strong>×${x.count}</strong>`:''}</article>`).join('');
  return `<section class="career-summary-head"><div><small>CAREER SUMMARY</small><h3>${escapeHTML(S.name)}</h3></div><p><b>${teams.length}</b> 支球隊<i></i><b>${total}</b> 項獎項</p></section><section class="report-section career-summary-section"><h3>效力球隊</h3><div class="summary-team-grid">${teamHTML||'<div class="career-empty">尚未留下球隊紀錄。</div>'}</div></section><section class="report-section career-summary-section"><h3>生涯獎項</h3><div class="summary-honor-list">${honorHTML||'<div class="career-empty">目前沒有正式獎項。</div>'}</div></section><button class="btn main legacy-open-btn" id="open-legacy-hall" type="button">全螢幕查看生涯總結</button>`;
}
function legacyEpilogueHTML(){const teams=careerTeamSummaryItems(),honors=careerHonorSummaryItems();return `<b>${S.name}</b><br>效力球隊｜${teams.map(x=>x.team).join('、')||'尚無正式球隊紀錄'}<br>生涯獎項｜${honors.length?honors.map(x=>`${x.title}${x.count>1?` ×${x.count}`:''}`).join('、'):'無'}`;}
...
function simSeason(lv){
  ...
  st.rawD=st.d;st.abilityBase=+(st.rawD-swing.total).toFixed(1);st.d=seasonCorePerformance(st,lv,st.rawD);st.performanceAdjustment=+(st.d-st.rawD).toFixed(1);
...
function seasonEffectHTML(st){
  const b=st.effectBreakdown||{},signed=v=>`${Number(v)>0?'+':''}${Number(v||0).toFixed(1)}`,rows=[];
  if(Number.isFinite(st.abilityBase))rows.push(['能力基準',signed(st.abilityBase)]);
  rows.push(['手感',signed(b.luck)],['事件',signed(b.momentum)],['選擇',signed(b.choice)],['環境',signed(b.environment)],['團隊',signed(b.chemistry)],['隊友',signed(b.teammates)],['特性',signed(b.traits)]);if(b.finance)rows.push(['財務壓力',signed(b.finance)]);
  ...
  if(Number.isFinite(b.total))rows.push(['狀態修正合計',signed(b.total)]);
  if(Number.isFinite(st.rawD))rows.push(['賽前評估',signed(st.rawD)]);
  if(Number.isFinite(st.d))rows.push(['本季實績',signed(st.d)]);
...
function seasonHealthLabel(st){
  const availability=clamp(Math.round(Number(st&&st.availability)||0),0,100),injury=(S.injuryHistory||[]).slice().reverse().find(x=>x&&x.year===S.year);
  if(availability>=90)return `健康 ${availability}%`;
  return injury?`${injury.title||'傷病影響'}・可出賽 ${availability}%`:`可出賽 ${availability}%`;
}
...
function openFx(id){ const el=$(id);modalReturnFocus=document.activeElement;el.classList.remove('finalizing','reveal-good','reveal-bad','reveal-jackpot','retirement-mode','achievement-mode','trade-mode');el.classList.add('open');el.setAttribute('aria-hidden','false');const machine=el.querySelector('.roll-machine,.season-machine,.summary-machine,.career-machine,.save-machine');if(machine)machine.scrollTop=0;document.body.style.overflow='hidden';requestAnimationFrame(()=>{const focus=el.querySelector('button:not([disabled]),[tabindex="0"]');if(focus)focus.focus({preventScroll:true});}); }
function closeFx(id){ const el=$(id),returnFocus=modalReturnFocus;modalReturnFocus=null;el.classList.remove('open','finalizing','reveal-good','reveal-bad','reveal-jackpot','retirement-mode','achievement-mode','trade-mode');el.setAttribute('aria-hidden','true');if(id==='roll-overlay'&&$('roll-stop'))$('roll-stop').style.display='none';document.body.style.overflow='';if(returnFocus&&document.contains(returnFocus))requestAnimationFrame(()=>{if(document.contains(returnFocus)&&typeof returnFocus.focus==='function')returnFocus.focus({preventScroll:true});}); }
let rollAudio=null;
...
const TRAINING_TRIPLE_REWARDS={
  1:{title:'恢復重整',preview:'本季傷病風險 −5%',injury:-5},
  2:{title:'基本功加練',preview:'追加 1 顆 2 點訓練骰',bonusDie:2},
  3:{title:'穩定節奏',preview:'追加 1 顆 3 點訓練骰',bonusDie:3},
  4:{title:'專項突破',preview:'追加 1 顆 4 點訓練骰、本季傷病風險 −2%',bonusDie:4,injury:-2},
  5:{title:'黃金訓練',preview:'追加 1 顆 6 點訓練骰',bonusDie:6},
  6:{title:'完美特訓',preview:'追加 1 顆 6 點訓練骰、隨機潛力上限 +2',bonusDie:6,potential:2}
};
function trainingTripleProfile(dice){
  const counts={};(dice||[]).forEach(v=>counts[v]=(counts[v]||0)+1);const face=[6,5,4,3,2,1].find(v=>(counts[v]||0)>=3);return face?{face,code:String(face).repeat(3),...TRAINING_TRIPLE_REWARDS[face]}:null;
}
function applyTrainingTripleReward(dice){
  const reward=trainingTripleProfile(dice);if(!reward)return {dice:dice.slice(),reward:null,text:''};const allocationDice=dice.slice();if(reward.bonusDie)allocationDice.push(reward.bonusDie);if(reward.injury)S.injNext=(S.injNext||0)+reward.injury;let potentialText='';
  if(reward.potential){const eligible=POS_AB[S.pos].filter(k=>Math.max(S.ab[k]||0,S.pot[k]||0)<RATING_MAX);if(eligible.length){const k=pick(eligible),before=Math.max(S.ab[k]||0,S.pot[k]||0),after=clamp(before+reward.potential,1,RATING_MAX);S.pot[k]=after;potentialText=`${ABL[k]} 潛力 ${before} → ${after}/99`;}else{S.injNext=(S.injNext||0)-5;potentialText='能力潛力已滿，改為本季傷病風險 −5%';}}
  const effects=[reward.bonusDie?`追加 ${reward.bonusDie} 點訓練骰`:'',reward.injury?`本季傷病風險 ${reward.injury}%`:'',potentialText].filter(Boolean);S.trainingTripleHistory=S.trainingTripleHistory||[];S.trainingTripleHistory.push({year:S.year,face:reward.face,effects:effects.slice()});return {dice:allocationDice,reward,text:effects.join('｜')};
}
function animatedDicePool(n,min,title){
  ...
  return new Promise(resolve=>{
    ...
    manualReels(els,dice,()=>Math.max(min||1,visualRi(1,6)),(stopped,total,braking,final)=>{$('roll-result').innerHTML=`<small>${final?'訓練成果鎖定中…':braking?`第 ${stopped+1} 顆正在減速…`:stopped?`${stopped}/${total} 顆已停下${stopped===total-1?'｜最後一顆':''}`:'點第一顆，或按下方按鈕開始'}</small>`;},()=>{
      ...
      const sixes=dice.filter(v=>v===6).length,total=dice.reduce((a,b)=>a+b,0),triple=trainingTripleProfile(dice);
      if(triple)els.forEach((el,i)=>{if(dice[i]===triple.face)el.classList.add('jackpot');});
      $('roll-result').className='roll-result '+(triple?'good jackpot':sixes?'good':'');$('roll-result').innerHTML=`<strong>${dice.join(' · ')}</strong><small>${triple?`三連獎 ${triple.code}｜${triple.title}<br>${triple.preview}`:`合計 ${total} 點`}</small>`;
      $('roll-overlay').classList.add(triple?'reveal-jackpot':'reveal-good');playRollSound('good');
      recordRoll({title:title||'季初訓練',display:String(total),outcome:triple?`${triple.code} 三連獎｜${triple.title}`:`${n} 顆共 ${total} 點`,probability:null,good:sixes>0||!!triple,bad:false});
...
function achievementFX(item){
  return new Promise(resolve=>{
    ...
    if(isTrade){
      const from=String(item.from||'原球隊'),to=String(item.to||'新球隊'),fromColor=TEAM_COLOR[from]||'#8b968f',toColor=TEAM_COLOR[to]||'#73c69a',fromCode=teamBroadcastCode(from),toCode=teamBroadcastCode(to);
      $('roll-overlay').style.setProperty('--from-team',fromColor);$('roll-overlay').style.setProperty('--to-team',toColor);
      $('roll-kicker').textContent=item.kicker||'TRANSACTION · OFFICIAL';
      $('roll-title').textContent=`${S&&S.name?S.name:'球員'} 轉戰 ${to}`;
      $('roll-slots').innerHTML=`<div class="trade-reveal" style="--from-team:${fromColor};--to-team:${toColor}"><span class="trade-status">ACQUIRED BY</span><div class="trade-destination"><em>${escapeHTML(toCode)}</em><div><small>新東家</small><strong>${escapeHTML(to)}</strong></div></div><div class="trade-ticker"><span><small>FROM</small><b>${escapeHTML(fromCode)}</b><em>${escapeHTML(from)}</em></span><i>TRADE</i><span><small>TO</small><b>${escapeHTML(toCode)}</b><em>${escapeHTML(to)}</em></span></div></div>`;
      window.__tradeFxPayload={from,to,fromCode,toCode,fromColor,toColor,player:S&&S.name?S.name:'球員',instant:new URLSearchParams(location.search).get('visual-audit')==='trade-fx'};
    }
    ...
    $('roll-formula').innerHTML=isTrade?`<div class="trade-fact"><small>合約</small><b>${escapeHTML(item.detail||'由新隊承接')}</b></div>${item.progress?`<div class="trade-fact"><small>交易背景</small><b>${escapeHTML(item.progress)}</b></div>`:''}`:`<span>${item.detail||'生涯條件達標'}</span>${item.progress?`<span>目前進度 <b>${item.progress}</b></span>`:''}`;
    $('roll-result').className=`roll-result good${isTrade?' trade-result':''}`;$('roll-result').innerHTML=isTrade?`<small>新隊角色</small><strong>${escapeHTML(item.note||'重新競爭名單席次')}</strong>`:`<strong>${item.result||item.title}</strong><small>${item.note||'已加入生涯履歷'}</small>`;$('roll-stop').style.display='none';$('roll-accept').style.display='block';
    openFx('roll-overlay');$('roll-overlay').classList.add('achievement-mode');if(isTrade){$('roll-overlay').classList.add('trade-mode');if(window.DiceFX&&window.DiceFX.trade)window.DiceFX.trade(window.__tradeFxPayload);}else if(window.DiceFX&&window.DiceFX.award)window.DiceFX.award(item.kind);
...
function tradeFX(info){
  ...
  return achievementFX({kind:'trade',kicker:info.kicker||'TRANSACTION · OFFICIAL',title:info.title||'交易正式生效',subtitle:`${S.year}｜${level}`,from:info.from,to:info.to,detail:contract,progress:info.reason||'',note:role});
}
...
function mergeSeasonSegments(parts){
  ...
  const weight=p=>Math.max(1,S.pos==='P'?(p.st.IP||0):(p.st.PA||0)),weightSum=parts.reduce((n,p)=>n+weight(p),0),weighted=k=>+(parts.reduce((n,p)=>n+(Number(p.st[k])||0)*weight(p),0)/Math.max(1,weightSum)).toFixed(1);st.avgVelo=st.IP?+(parts.reduce((n,p)=>n+(p.st.avgVelo||0)*(p.st.IP||0),0)/st.IP).toFixed(1):0;st.d=weighted('d');st.rawD=weighted('rawD');st.abilityBase=weighted('abilityBase');st.performanceAdjustment=+(st.d-st.rawD).toFixed(1);
  ...
  st.variance=+(parts.reduce((n,p)=>n+(Number(p.st.variance)||0)*weight(p),0)/Math.max(1,weightSum)).toFixed(2);st.varianceLabel=st.variance>=1.35?'整季狀態偏熱':st.variance<=-1.35?'整季狀態偏冷':'';st.varianceKind=st.variance>=1.35?'hot':st.variance<=-1.35?'cold':'';const outlierParts=parts.filter(p=>p.st.outlierKind),hotOutliers=outlierParts.filter(p=>p.st.outlierKind==='hot').length,coldOutliers=outlierParts.filter(p=>p.st.outlierKind==='cold').length,outlierNeed=Math.max(2,Math.ceil(parts.length/2));st.outlierKind=hotOutliers>=outlierNeed?'hot':coldOutliers>=outlierNeed?'cold':'';st.outlierLabel=st.outlierKind==='hot'?'多月爆發':st.outlierKind==='cold'?'多月低潮':'';st.traitImpact=parts.flatMap(p=>p.st.traitImpact||[]);
  ...
  let afterAsk=()=>{
    ...
    choose('季初自主訓練',[{t:`開始 ${n} 顆訓練拉霸`,main:true,s:`${n} 顆｜每顆 1～6`,f:()=>{
      animatedDicePool(n,floor,`${S.year} 季初訓練骰`).then(dice=>{
        const tripleResult=applyTrainingTripleReward(dice),allocationDice=tripleResult.dice;let newSix=0;
        ...
        if(tripleResult.reward)msg+=`<div class="statline"><b>${tripleResult.reward.code} 三連獎｜${tripleResult.reward.title}</b><br>${tripleResult.text}</div>`;
        card(tripleResult.reward?'gold':'','季初特訓',msg);
        ...
        dposReview(()=>allocUI({dice:allocationDice},'分配訓練成果｜能力上限 99',()=>nextStep()));
      });
    }}]);
  };
  /* 投手開季：投球強度(續航+TJ 量表) */
...
function proSeason(){
  ...
  { const b=st.effectBreakdown||{},luckText=S.seasonLuck===20?'生涯年':S.seasonLuck>=16?'火燙':S.seasonLuck>=7?'正常':S.seasonLuck>=2?'低潮':'崩盤',signed=v=>`${Number(v)>0?'+':''}${Number(v||0).toFixed(1)}`;
    card(b.total>=2?'good':b.total<=-2?'bad':'info','球季變數拆解',
      `<span class="tag">${st.usageRole}</span><span class="tag">${seasonHealthLabel(st)}</span><span class="tag">出賽 ${st.G}/${st.scheduled}</span><span class="tag">狀態 ${S.seasonLuck}/20・${luckText}</span><div class="statline">本季實績 <b>${signed(st.d)}</b>｜賽前評估 <b>${signed(st.rawD)}</b></div><details class="report-details"><summary>查看影響明細</summary>${seasonEffectHTML(st)}</details><small>賽前評估由能力與狀態組成；本季實績依實際成績結算。</small>${S.pos!=='P'?contractMarketResume():''}`); }
...
function currentSeasonPromotionGrace(){
  const r=S._callupReview;return !!(r&&r.year===S.year&&r.success&&r.to===S.lv);
}
function postSeasonReviewProfile(st){
  const firstTeamReview=firstTeamPerformanceReview(st);
  if(firstTeamReview.firstTeam)return currentSeasonPromotionGrace()&&!firstTeamReview.severe?{...firstTeamReview,needs:false,promotionGrace:true}:firstTeamReview;
  ...
  const severe=actualRosterVerdict(st).awful,promotionGrace=currentSeasonPromotionGrace()&&!severe;
  return {needs:promotionGrace?false:needs,sample,poor,borderline,finalChance,severe,line,team,promotionGrace};
}
function postSeasonRosterMeeting(done){
  ...
  const opts=[
    {t:'接受縮減角色，明年重新競爭',main:true,modal:true,s:'團隊關係 +1｜球團保留調整名單的權利',f:()=>{S.chemistry=clamp((S.chemistry||0)+1,-5,5);finish('accept',8,'info','季後約談｜接受球團方案','你接受明年從較低順位重新競爭。這能保住關係，最終層級仍由球團依戰力決定。');}},
...
function movement(){
  ...
  else if(promotionGrace){
    card('info','升格後留隊觀察',`本季才完成升格，不會用升格後的短期樣本立刻反向下放。明年將從目前層級正常競爭。<div class="statline">目前層級｜${LV[S.lv].n}<br>升格後成績｜${rosterVerdict.line}</div>`);
  }
...
if(new URLSearchParams(location.search).get('visual-audit')==='trade-fx'){
  RNG_MODE='destiny';seedInit('visual-audit-trade-fx');S=attachStateMethods(newState('蔣孟杰','P','MR'));S.stage='PRO';S.lv='A3';S.org='MiLB';S.orgTeam='西雅圖水手';S.dpos='P';S.age=24;S.year=2034;S.stageYr=8;S.ct={yrs:2,signedYears:4,annual:159,mult:1.08,guaranteed:1};showGameShell();board(0);tradeFX({from:'聖地牙哥教士',to:'西雅圖水手',level:'3A',reason:'球團在季後名單會議完成正式交易',contract:'原合約剩 2 年・年薪 US$50,000（約 159萬台幣） 由新隊承接',role:'牛棚的季後賽優先席次；低潮時仍會縮短觀察期'});
}
...
if(new URLSearchParams(location.search).get('visual-audit')==='training-jackpot'){
  RNG_MODE='destiny';seedInit('visual-audit-training-jackpot');S=attachStateMethods(newState('蔣孟杰','IF',null));S.stage='PRO';S.lv='CPBL1';S.org='CPBL';S.orgTeam='台中猛瑪';S.dpos='SS';S.age=22;S.year=2042;$('start').style.display='none';document.body.classList.add('game-started');$('board').style.display='';$('act').style.display='';$('player-rail').style.display='';$('roll-rail').style.display='';board(1);$('roll-kicker').textContent='訓練拉霸';$('roll-title').textContent='2042 季初訓練骰';$('roll-sub').textContent='依序點亮起的骰子，或按下方按鈕，一顆一顆停下。';$('roll-slots').innerHTML='<div class="pool-dice"><div class="pool-die stopped jackpot">6</div><div class="pool-die stopped jackpot">6</div><div class="pool-die stopped jackpot">6</div></div>';$('roll-formula').innerHTML='<span><b>3</b> 顆獨立訓練骰</span><span>1～6 各 <b>1/6</b></span>';$('roll-result').className='roll-result good jackpot';$('roll-result').innerHTML='<strong>6 · 6 · 6</strong><small>三連獎 666｜完美特訓<br>追加 1 顆 6 點訓練骰、隨機潛力上限 +2</small>';$('roll-stop').style.display='none';$('roll-accept').style.display='block';openFx('roll-overlay');$('roll-overlay').classList.add('reveal-jackpot');
}
...
function runLogicAudit(samples){
  ...
  setup('IF','CPBL1',null,'SS');S.year=2032;S.age=22;S._callupReview={year:S.year,from:'CPBL2',to:'CPBL1',success:true,monthIndex:2};const shortCallup={G:35,PA:71,AB:62,H:14,BB:7,_1B:9,_2B:5,_3B:0,HR:0,avg:14/62,d:-1.1},shortReview=postSeasonReviewProfile(shortCallup),disasterCallup={G:40,PA:120,AB:110,H:10,BB:6,_1B:9,_2B:1,_3B:0,HR:0,avg:10/110,d:-8},disasterReview=postSeasonReviewProfile(disasterCallup);result.promotionGrace={recognized:currentSeasonPromotionGrace(),shortFirstTeamSampleDoesNotTriggerMeeting:shortReview.promotionGrace&&!shortReview.needs,disasterStillReviewed:disasterReview.severe&&disasterReview.needs};
  setup('IF','CPBL1',null,'SS');Object.keys(S.ab).forEach(k=>{S.ab[k]=65;S.pot[k]=75;});seedInit('audit-training-triple');const beforeTriplePot=Math.max(...POS_AB[S.pos].map(k=>S.pot[k])),tripleProfiles=[1,2,3,4,5,6].map(v=>trainingTripleProfile([v,v,v])),sixTriple=applyTrainingTripleReward([6,6,6]),afterTriplePot=Math.max(...POS_AB[S.pos].map(k=>S.pot[k]));result.trainingTriple={allFacesCovered:tripleProfiles.every((x,i)=>x&&x.face===i+1),nonTripleIgnored:trainingTripleProfile([6,5,6])===null,sixAddsBonusDie:sixTriple.dice.length===4&&sixTriple.dice[3]===6,sixRaisesOnePotential:afterTriplePot===beforeTriplePot+2,historyRecorded:S.trainingTripleHistory.length===1};
  setup('IF','CPBL1',null,'SS');Object.assign(S.ab,{con:68,pow:62,eye:66,spd:65,sta:70,rng:67,fld:68,arm:66});seedInit('audit-season-effect-reconcile');const effectLine=simSeason('CPBL1'),effectExpected=effectLine.abilityBase+effectLine.effectBreakdown.total;result.seasonEffectReconciliation={abilityBase:effectLine.abilityBase,externalTotal:effectLine.effectBreakdown.total,preseason:effectLine.rawD,actual:effectLine.d,reconciles:Math.abs(effectExpected-effectLine.rawD)<=.2,actualSeparatedFromForecast:Number.isFinite(effectLine.performanceAdjustment)&&Math.abs((effectLine.d-effectLine.rawD)-effectLine.performanceAdjustment)<=.11};
  ...
  setup('IF','A1',null,'SS');seedInit('audit-mlb-npc-names');prepareNpcSeason();const mlbNpcNames=Object.values(npcWorld().rosters).flat().map(p=>p.name);result.npcNameFormat={generated:mlbNpcNames.length,unique:new Set(mlbNpcNames).size===mlbNpcNames.length,noNumericSuffix:mlbNpcNames.every(name=>!/\s\d{2}$/.test(name)),legacySerialRepair:repairNpcSerialName('Caleb Moreno 65')==='Caleb N. Moreno'&&repairNpcSerialName('Dylan Cruz 99')==='Dylan V. Cruz'};
